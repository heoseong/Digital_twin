...
fit1_1 = lmer( brix ~ monthweek + ( 1 | farm_id ), data = data.new7 )
summary(fit1_1); r.squaredGLMM(fit1_1)

### FRUIT SIZE

fit1_2 = lmer( size ~ monthweek + ( 1 | farm_id ), data = data.new7 )
summary(fit1_2); r.squaredGLMM(fit1_2)

### adjusted p-values
p.adjust( c( summary(fit1_1)$coef[-1,5], summary(fit1_2)$coef[-1,5] ), method = "BY" )
