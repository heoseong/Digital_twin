...
jpeg( "Figures/Fig3A.jpg", width = 500, height = 400, quality = 100 )
...
temp3$brix_col = cut( temp3$brix_ave, 
                      breaks = c( 0, 9.5, 10.5, 11.5, Inf ),
                      labels = paste0( "orange", 1:4 ) )
...
temp5$brix_col = as.character(temp5$brix_col)

par( new = TRUE )
plot( temp5$longitude, temp5$latitude,
      xlim = c(126.15,127), ylim = c(33.1,33.575),
      xlab = "", ylab = "", axes = FALSE,
      pch = 19, col = temp5$brix_col, cex = temp5$size_ave / 100 * 3 )

text( x = 126.9, y = 33.2, labels = "Sugar content (°Bx)", font = 1, cex = 0.9 )
legend( x = 126.8, y = 33.195, pch = 19, pt.cex = 1.5, col = paste0( "orange", 1:4 ),
        legend = c( "<9.5", "9.5-10.5", "10.5-11.5", ">11.5" ), bty = "n", cex = 0.9 )

text( x = 126.6, y = 33.2, adj = 0, labels = "Fruit size (mm)", font = 1, cex = 0.9 )
legend( x = 126.6, y = 33.195, pch = 19, 
        pt.cex = c( 40, 50, 60, 70 ) / 100 * 3, col = 4,
        legend = c( "40", "50", "60", "70" ), bty = "n", cex = 0.9 )
...
jpeg( "Figures/Fig3B.jpg", width = 500, height = 400, quality = 100 )
...
temp4 = temp4[ duplicated(temp4$farm_id) == FALSE, ]
temp4 = temp4[ , c( "farm_id", "latitude", "longitude" ) ]
temp5 = temp.nov12 = merge( temp3, temp4, by = "farm_id" )
temp5$brix_col = as.character(temp5$brix_col)
...
jpeg( "Figures/Fig3C.jpg", width = 500, height = 400, quality = 100 )
...
temp4 = temp4[ duplicated(temp4$farm_id) == FALSE, ]
temp4 = temp4[ , c( "farm_id", "latitude", "longitude" ) ]
temp5 = temp.nov34 = merge( temp3, temp4, by = "farm_id" )
temp5$brix_col = as.character(temp5$brix_col)
...
temp.nov12$time = 2
temp.nov34$time = 3
temp.all = rbind( temp.oct45, temp.nov12, temp.nov34 )
temp.all = temp.all[ order( temp.all$farm_id, temp.all$time ),]
temp.all$region = "NE"
temp.all$region[ temp.all$latitude > 33.4 & temp.all$longitude < 126.5 ] = "NW"
temp.all$region[ temp.all$latitude < 33.4 & temp.all$longitude < 126.5 ] = "SW"
temp.all$region[ temp.all$latitude < 33.4 & temp.all$longitude > 126.5 ] = "SE"

temp.all$farm_id = paste0( toupper( substr( temp.all$farm_id, 1, 1 ) ),
                           substr( temp.all$farm_id, 2, 3 ) )

jpeg( "Figures/Fig3D.jpg", width = 420, height = 420, quality = 100 )

par( mfrow = c(1,1) )
par( mar = c(2.1, 2.1, 3.1, 2.1) ) 

### SW

temp6 = subset( temp.all, region == "SW" )
u = unique(temp6$farm_id)

i = 1; temp7 = subset( temp6, farm_id == u[i] )
plot( 1, 1, xlim = c(0.5,5.5), ylim = c(0,28), col = "white",
      xlab = "", ylab = "", axes = FALSE )
axis( 3, c( -10, 1:5, 10 ), tick = FALSE,
      c( "", "Orchard", "Region", "Oct 4-5", "Nov 1-2", "Nov 3-4", "" ), font = 2 )
...
for (i in 1:length(u) ) {
  temp7 = subset( temp6, farm_id == u[i] )
  par( new = TRUE )
  plot( temp7$time + 2, rep( count, length(temp7$time) ), xlim = c(0.5,5.5), ylim = c(0,27),
        pch = 19, col = as.character(temp7$brix_col), cex = temp7$size_ave / 100 * 2,
        xlab = "", ylab = "", axes = FALSE )
  text( x = 1, y = count, label = u[i], cex = 0.8  )
  text( x = 2, y = count, label = "SW", cex = 0.8 )
  count = count + 1
}

abline( h = count - 0.5, col = 8 )

### SE

temp6 = subset( temp.all, region == "SE" )
u = unique(temp6$farm_id)
...
for (i in 1:length(u) ) {
  ...
  plot( temp7$time + 2, rep( count, length(temp7$time) ), xlim = c(0.5,5.5), ylim = c(0,27),
        ...
        xlab = "", ylab = "", axes = FALSE )
  text( x = 1, y = count, label = u[i], cex = 0.8  )
  text( x = 2, y = count, label = "SE", cex = 0.8 )
  count = count + 1
}
...
temp6 = subset( temp.all, region == "NE" )
u = unique(temp6$farm_id)
...
for (i in 1:length(u) ) {
  ...
  plot( temp7$time + 2, rep( count, length(temp7$time) ), xlim = c(0.5,5.5), ylim = c(0,27),
        ...
        xlab = "", ylab = "", axes = FALSE )
  text( x = 1, y = count, label = u[i], cex = 0.8  )
  text( x = 2, y = count, label = "NE", cex = 0.8 )
  count = count + 1
}
...
temp6 = subset( temp.all, region == "NW" )
u = unique(temp6$farm_id)
...
for (i in 1:length(u) ) {
  ...
  par( new = TRUE )
  plot( temp7$time + 2, rep( count, length(temp7$time) ), xlim = c(0.5,5.5), ylim = c(0,27),
        pch = 19, col = as.character(temp7$brix_col), cex = temp7$size_ave / 100 * 3,
        xlab = "", ylab = "", axes = FALSE )
  text( x = 1, y = count, label = u[i], cex = 0.8  )
  text( x = 2, y = count, label = "NW", cex = 0.8 )
  count = count + 1
}
