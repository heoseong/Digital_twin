...
fit2_1 = lmer( brix ~ monthweek + position + ( 1 | farm_id ) + ( 1 | tree_id ), data = data.new7 )
summary(fit2_1); r.squaredGLMM(fit2_1)

### FRUIT SIZE

fit2_2 = lmer( size ~ monthweek + position + ( 1 | farm_id ) + ( 1 | tree_id ), data = data.new7 )
summary(fit2_2); r.squaredGLMM(fit2_2)

### adjusted p-values
p.adjust( c( summary(fit2_1)$coef[-1,5], summary(fit2_2)$coef[-1,5] ), method = "BY" )
